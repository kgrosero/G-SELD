...
class DataGenerator(object):
    def __init__(
        ...
        self._feat_cls = cls_feature_class_ov3.FeatureClass(params=params, is_eval=self._is_eval)
        self._label_dir = self._feat_cls.get_label_dir()
        self._feat_dir = self._feat_cls.get_normalized_feat_dir()

        self._filenames_list = list()
        self._nb_frames_file = 0     # Using a fixed number of frames in feat files. Updated in _get_label_filenames_sizes(), _get_filenames_list_and_feat_label_sizes
        ...
        #print(self._get_filenames_list_and_feat_label_sizes)

	#self._feature_batch_seq_len = 256*300=76800
        self._feature_batch_seq_len = self._batch_size*self._feature_seq_len
        #self._label_batch_seq_len = 256*60 = 15360
        self._label_batch_seq_len = self._batch_size*self._label_seq_len
        self._circ_buf_feat = None
        self._circ_buf_label = None
        print(len(self._filenames_list),self._nb_frames_file,self._feature_batch_seq_len)
    ...
    def get_data_sizes(self):
        ...
        if self._is_eval:
            ...
            if self._is_accdoa:
                label_shape = (self._batch_size, self._label_seq_len, self._nb_classes*9)
            else:
                label_shape = [
                    (self._batch_size, self._label_seq_len, self._nb_classes*3),
                    (self._batch_size, self._label_seq_len, self._nb_classes*9) 
                ]
    ...
    def _get_filenames_list_and_feat_label_sizes(self):
        ...
        for filename in os.listdir(self._feat_dir):
            #print('filename',filename, int(filename[4:6]))
            if self._is_eval:
                self._filenames_list.append(filename)
            else:
                if int(filename[4:6]) in self._splits: # check which split the file belongs to
                    #print('fold ', filename)
                    self._filenames_list.append(filename)
        ...
        #print('self._nb_frames_file', self._nb_frames_file)
        ...
        if not self._is_eval:
            ...
            self._label_len = temp_label.shape[-1] #144
            #print('self._label_len', self._label_len)
            self._doa_len = (self._label_len - 3*self._nb_classes)//(3*self._nb_classes) #3
                ...
                for i in range(self._nb_total_batches):
                    ...
                    while len(self._circ_buf_feat) < self._feature_batch_seq_len:
                        ...
                        temp_label = np.load(os.path.join(self._label_dir, self._filenames_list[file_cnt]))
                            ...
                            #print('temp_feat len:', self._filenames_list[file_cnt])
                            #print('temp_label len:',len(temp_label))
                        ...
                        if self._per_file:
                            ...
                            for f_row in extra_feat:
                                self._circ_buf_feat.append(f_row)
                                #print('f_row', f_row.shape, f_row)
                            for l_row in extra_labels:
                                self._circ_buf_label.append(l_row)
                                #print('l_row', l_row.shape, l_row)
			# entra a perfile pero como no hay frames extras, queda en cero
                        file_cnt = file_cnt + 1
                    ...
                    feat = np.reshape(feat, (self._feature_batch_seq_len, self._nb_ch, self._nb_mel_bins)).transpose((0, 2, 1))
                    #if not self._shuffle:
                        #print('feat, label shape antes:', feat.shape, label.shape)
                    ...
                    #if not self._shuffle:
                        #print('feat, label shape despues:', feat.shape, label.shape)
                    
                    if self._is_accdoa:
                        #print(label.shape)
                        mask = label[:, :, :self._nb_classes*3]
                        #if not self._shuffle:
                            #print('mask1:', mask.shape, mask[0,59,:])
                        mask = np.tile(mask, 3) # para agregar las dos posiciones mas para cada clase, por eso en mask1 tenemos solo 12 posiciones y luego 36
                        #if not self._shuffle:
                            #print('mask2:', mask.shape, mask[0,59,:])                        
                        #print('mask2: ', mask[120][58])
                        label = mask * label[:, :, 3*self._nb_classes:]
                        #print('mask3: ', label[120][58])
                        #if not self._shuffle:
                            #print('mask*label:', label.shape, label[0,59,:])   
                    else:
                         label = [
                            label[:, :, :self._nb_classes*3],  # SED labels
                            label[:, :, self._nb_classes*3:] if self._doa_objective is 'mse' else label # SED + DOA labels
                             ]
                    yield feat, label
    ...
    def _split_in_seqs(self, data, _seq_len):
        if len(data.shape) == 1:
            ...
            if data.shape[0] % _seq_len: #to known if 76800%300 is an exact division
                data = data[:-(data.shape[0] % _seq_len), :, :]
